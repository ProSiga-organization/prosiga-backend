...
class AvisoRepository:
    ...
    def create_aviso_curso(
        self, db: Session, request: schema.AvisoCursoCreate, id_autor: int
    ) -> model.Aviso:
        """
    ...
    def get_aviso_by_id(self, db: Session, id_aviso: int) -> Optional[model.Aviso]:
        ...
        return (
            ...
            .filter(model.Aviso.id == id_aviso)
            .first()
        )

    def update_aviso(
        self, db: Session, aviso_db: model.Aviso, request: schema.AvisoUpdate
    ) -> model.Aviso:
        """
