...
@router.get(
    ...
    summary="Lista as matrículas do aluno logado",
)
...
def trancar_disciplina(
    ...
    if not periodo_letivo or not periodo_letivo.fim_trancamento:
        raise HTTPException(
            status_code=400,
            detail="Prazo de trancamento não definido pela administração.",
        )
...
def admin_create_matricula(
    ...
    matricula_existente = repo.get_by_aluno_and_turma(
        db, id_aluno=aluno.id, id_turma=request.id_turma
    )
    if matricula_existente:
        raise HTTPException(status_code=409, detail="Aluno já matriculado nesta turma.")
...
def create_or_update_nota_celula(
    ...
    if not avaliacao_db:
        raise HTTPException(
            status_code=404, detail="Avaliação (coluna) não encontrada."
        )
    ...
    if avaliacao_db.turma.id_professor != current_professor.id:
        raise HTTPException(
            status_code=403,
            detail="Professor não tem permissão para lançar notas nesta avaliação.",
        )
    ...
    matricula_db = repo.get_by_aluno_and_turma(
        db, id_aluno=id_aluno_encontrado, id_turma=id_turma_da_avaliacao
    )
    if not matricula_db:
        raise HTTPException(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Erro ao salvar a nota: {e}"
        ) from e
