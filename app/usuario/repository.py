...
class UsuarioRepository:
    ...
    def ativar_conta(
        self,
        db: Session,
        usuario_db: model.Usuario,
        dados_ativacao: schema.PrimeiroAcessoSchema,
    ) -> model.Usuario:
        """
    ...
    def get_aluno_by_matricula(self, db: Session, matricula: str) -> model.Aluno | None:
        ...
        """
        return (
            db.query(model.Aluno)
            .filter(model.Aluno.matricula == str(matricula))
            .first()
        )

    def set_usuario_status(
        self, db: Session, usuario_db: model.Usuario, novo_status: model.StatusContaEnum
    ) -> model.Usuario:
        """
    ...
    def get_all_professores(self, db: Session) -> list[model.Professor]:
        ...
        """
        return db.query(model.Professor).all()
