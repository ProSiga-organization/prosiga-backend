    ...
    try:
        ...
        for row_num, row in enumerate(csv_reader, start=2):
            ...
            if tipo_usuario == "aluno":
                ...
                if codigo_curso:
                    ...
                    if not id_curso:
                        erros_importacao.append(
                            f"Linha {row_num} (CPF {cpf}): Código de curso '{codigo_curso}' inválido."
                        )
                        continue

                if not matricula:
                    erros_importacao.append(
                        f"Linha {row_num} (CPF {cpf}): Matrícula em falta para aluno."
                    )
                    continue

                novo_usuario = model.Aluno(
                    cpf=cpf,
                    nome=nome,
                    matricula=matricula,
                    senha_hash="",
                ...
                novo_usuario = model.Professor(
                    cpf=cpf, nome=nome, senha_hash="", status=model.StatusContaEnum.NOVO
                )
            elif tipo_usuario == "coordenador":
                novo_usuario = model.Coordenador(
                    cpf=cpf, nome=nome, senha_hash="", status=model.StatusContaEnum.NOVO
                )
            else:
                erros_importacao.append(
                    f"Linha {row_num} (CPF {cpf}): Tipo de usuário '{tipo_usuario}' inválido."
                )
                continue
        ...
        if novos_usuarios:
            ...
            db.commit()
            response_message += (
                f"{len(novos_usuarios)} novos usuários pré-cadastrados com sucesso! "
            )
        else:
...
def desativar_usuario(
    ...
    if not usuario_db:
        raise HTTPException(
            status_code=404, detail="Usuário não encontrado com este CPF."
        )

    if usuario_db.cpf == current_coordenador.cpf:
        raise HTTPException(
            status_code=400, detail="Não é permitido desativar a própria conta."
        )

    usuario_desativado = repo.set_usuario_status(
        db, usuario_db=usuario_db, novo_status=model.StatusContaEnum.INATIVO
    )
...
@router.get(
    "/me/historico-pdf", summary="Gera o histórico acadêmico do aluno logado em PDF"
)
def get_historico_pdf(
    ...
    matriculas = (
        ...
        .options(joinedload(model.Matricula.turma).joinedload(model.Turma.disciplina))
        .filter(model.Matricula.id_aluno == current_aluno.id)
        .all()
    )
...
def get_historico_aluno_admin(
    ...
    matriculas = (
        db.query(model.Matricula)
        .options(joinedload(model.Matricula.turma).joinedload(model.Turma.disciplina))
        .filter(model.Matricula.id_aluno == aluno.id)
        .all()
    )
    semestre_atual = repo_matricula.get_periodos_cursados_por_aluno(
